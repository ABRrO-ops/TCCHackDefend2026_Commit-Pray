...
router.post('/demande', async (req, res) => {
    try {
        const { nom_microfinance, ville, telephone, numero_agrement, nom_directeur,
            prenom_directeur, email_directeur, plan_choisi } = req.body;
        await pool.query(` 
INSERT INTO demandes_inscription 
(nom_microfinance, ville, telephone, numero_agrement, nom_directeur, 
prenom_directeur, email_directeur, plan_choisi, statut) 
VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 'en_attente') 
`, [nom_microfinance, ville, telephone, numero_agrement, nom_directeur,
            prenom_directeur, email_directeur, plan_choisi]);
...
// GET /api/inscription/demandes (votre back-office, protégé) 
router.get('/demandes', verifyToken, async (req, res) => {
    try {
        const result = await pool.query(` SELECT * FROM demandes_inscription WHERE statut = 'en_attente' ORDER BY created_at ASC `);
...
// POST /api/inscription/valider/:id (votre back-office) 
router.post('/valider/:id', verifyToken, async (req, res) => {
    try {
        ...
        // Générer le domaine email (ex: "CECAV Fraternité" -> "cecav") 
        const domaineEmail = d.nom_microfinance.split(' ')[0].toLowerCase().replace(/[^az]/g, '');
        // Créer la micro-finance 
...
`, [d.nom_microfinance, d.ville, d.telephone, d.plan_choisi, domaineEmail]);
        ...
        // Générer email + mot de passe temporaire pour le directeur 
        const emailGenere = await genererEmailInterne(d.prenom_directeur,
            d.nom_directeur, domaineEmail);
...
`, [d.nom_directeur, d.prenom_directeur, emailGenere, hash, microfinanceId]);
        await pool.query(`UPDATE demandes_inscription SET statut = 'validee' WHERE id = 
$1`, [id]);
        res.json({
...
router.post('/rejeter/:id', verifyToken, async (req, res) => {
    try {
        const { id } = req.params;
        await pool.query(`UPDATE demandes_inscription SET statut = 'rejetee' WHERE id = 
$1`, [id]);
